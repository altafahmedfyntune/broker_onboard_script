...
`broker_id` INTEGER,
`instance_mechanism` TEXT CHECK(instance_mechanism IN ('broker onboard','docker')),
`server_id` TEXT,
`environment_id` INTEGER,
`domain_name` TEXT,
`frontend_pos_url` TEXT,
`frontend_nonpos_url` TEXT,
`frontend_url_description` TEXT,
`backend_pos_url` TEXT,
`backend_nonpos_url` TEXT,
`backend_url_description` TEXT,
`ckyc_url` TEXT,
`ckyc_url_description` TEXT,
`instance_creation_status` TEXT CHECK(instance_creation_status IN ('Y','N')),
`created_at` TIMESTAMP,
`updated_at` TIMESTAMP,
`deleted_at` TIMESTAMP,
`deployment_status` TEXT CHECK(deployment_status IN ('in_queue','started','completed','failed'))
);
